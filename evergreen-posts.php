...
function evergreen_posts_enqueue_scripts() {
...
function evergreen_posts_add_button() {
    ...
    // Check if the current post is already marked as evergreen
    $evergreen_class = '';
    $evergreen_text = 'Evergreen';
    if (get_post_meta($post->ID, 'evergreen_post', true)) {
        $evergreen_class = 'evergreen-active';
        $evergreen_text = 'Evergreen';
    }
    ...
    echo '<input type="checkbox" name="evergreen_post" id="evergreen-post" value="1" ' . checked(get_post_meta($post->ID, 'evergreen_post', true), 1, false) . '>';
    echo 'Evergreen';
    echo '</label>';
    echo '</div>';
}
